  ...
  return (
    <div className="modal-overlay" onClick={onClose}>
      <div className="modal" onClick={(e) => e.stopPropagation()}>
        ...
        <div className="tailors-measurements-grid">
          {measurements && Object.entries(measurements).map(([key, val]) => val ? (
            <div key={key} className="tailors-measurement-item">
              <span className="tailors-measurement-label">{key.charAt(0).toUpperCase() + key.slice(1)}</span>
              <span className="tailors-measurement-value">{val} cm</span>
            </div>
          ) : null)}
        </div>
